...
def load_data():
    ...
    for row in r:
        ...
        row_count += 1
        properties = { (properties_to_keep[k]):v for (k,v) in row.items()
        ...
        if row['PRIM_LONG_DEC'] and row['PRIM_LONG_DEC']:
            ...
            if (long, lat) == (0, 0):
                ...
                with_prim += 1
                yield geojson(long, lat, properties)
    logging.debug('row count %s, with source %s, with prim %s, null source %s, null prim %s', row_count, with_source, with_prim, null_source, null_prim)
